...
def load_word_set():
    file_object = open('./corpus/chatsegment.pair', 'r',encoding='utf-8')
    while True:
        ...
        if line:
            ...
            break
    #print("load word finished",count)
    file_object.close()
...
def load_vectors(input):
    ...
    print("words_and_size=",words_and_size)
    #byte转为str decode,反之encode
    words = words_and_size.decode('utf-8').split(' ')[0]
    size = words_and_size.decode('utf-8').split(' ')[1]
    words =int(words)
    size =int(size)
    print ("words =", words)
    print ("size =", size)
    ...
    for b in range(0, words):
        ...
        # 读取一个词
        while True:
            #c为byte，byte to str decode, word 为str
            c = input_file.read(1)
            c = c.decode('unicode_escape').encode('utf-8')
            c = c.decode('utf-8')
            #print("c=",c)
            word = word + c
            #print("word=",word)
        ...
        if word in word_set:
            ...
            #print("word_vector_dict=",word_vector_dict[word])
            #print("word=",word)
            ...
            if  phrase == 'train':
                ...
                for word in line_answer.split(' '):
                    if word in word_vector_dict.keys():
                        ...
                        #print("answer_seq=",answer_seqs)
        else:
            break
        question_seqs.append(question_seq)
        #print("question_seqs=",question_seqs)
        answer_seqs.append(answer_seq)
    print("init seq finished")
    file_object.close()
...
class myGAN(object):
    ...
    """
    def __init__(self, max_seq_len = 16, word_vec_dim = 200, input_file='./corpus/chatsegment.pair'):
    ...
    def generate_trainig_data(self):
        load_word_set()
        load_vectors("./corpus/chat.vector")
        ...
        #for i in range(100):
            ...
            if len(question_seq) < self.max_seq_len and len(answer_seq) < self.max_seq_len:
                ...
                #print("xy_data=",xy_data,"y_data=",y_data)
        ...
        print("generate traindata finished")
    ...
    def model(self, feed_previous=False):
        ...
        #net=tflearn.input_data([None,self.word_vec_dim])
        #print("inputdata=",input_data)
        encoder_inputs = tf.slice(input_data, [0, 0, 0], [-1, self.max_seq_len, self.word_vec_dim], name="enc_in")
        #print("encoder_inputs=",encoder_inputs)
        decoder_inputs_tmp = tf.slice(input_data, [0, self.max_seq_len, 0], [-1, self.max_seq_len-1, self.word_vec_dim], name="dec_in_tmp")
        #print("decoder_inputs_tmp=",decoder_inputs_tmp)
        go_inputs = tf.ones_like(decoder_inputs_tmp)
        #print("input1=",go_inputs)
        go_inputs = tf.slice(go_inputs, [0, 0, 0], [-1, 1, self.word_vec_dim])
        #print("input2=",go_inputs)
        #print("inputs finish")
        decoder_inputs = tf.concat([go_inputs, decoder_inputs_tmp], axis=1, name="dec_in")
        #print("decoder=",decoder_inputs)
        #print("decoder_inputs generate")


        # 编码器
        # 把encoder_inputs交给编码器，返回一个输出(预测序列的第一个值)和一个状态(传给解码器)
        (encoder_output_tensor, states) = tflearn.lstm(encoder_inputs, self.word_vec_dim, return_state=True, scope='encoder_lstm')
        #tflearn.embedding(net,input_dim=40000,output_dim=self.word_vec_dim)
...
if __name__ == '__main__':
    phrase = sys.argv[1]
    if 3 == len(sys.argv):
        my_gan = myGAN(word_vec_dim=word_vec_dim, max_seq_len=max_seq_len, input_file=sys.argv[2])
    else:
        my_gan = myGAN(word_vec_dim=word_vec_dim, max_seq_len=max_seq_len)
    if phrase == 'train':
        my_gan.train()
    else:
        model = my_gan.load()
        testXY, testY = my_gan.generate_trainig_data()
        print("testXY=",testXY,"testY=",testY)
